...
Agent::Agent ()
{
    worldState.worldSize = 5;
	// HW5
	firstTry = true;
    for(int i = 0; i < worldState.worldSize; i++){
        for(int j = 0; j < worldState.worldSize; j++){
            pitProbability[i][j] = .2;
        }
    }

    pitProbability[0][0] = 0;
}
...
void Agent::UpdateState (Percept& percept) {
	...
	if (percept.Stench) {
        ...
        AddNewLocation(breezeLocations, worldState.agentLocation);
    } else {
...
void Agent::UpdatePossiblePitLocations() {
    list<Location> tmpLocations;
    list<Location> adjacentLocations;

    list<Location>::iterator clearItr;
    for (clearItr = safeLocations.begin(); clearItr != safeLocations.end(); ++clearItr){
        Location location = *clearItr;
        pitProbability[location.X][location.Y] = 0;
    }


}

list<Location> Agent::getNeighbors(Location loc){
    list<Location> result;

    for(int i = -1; i < 2; i+=2){
        for(int j = -1; j < 2; j+=2){
            Location curLoc = Location(loc.X + i, loc.Y + j);
            if(isInBounds(curLoc)){
...
bool Agent::listContains(list<Location> _list, Location loc){
...
bool Agent::IsValidPitArrangement(list<Location> pits){

    list<Location> lBreezes = list<Location>(breezeLocations);

    list<Location>::iterator pitItr;

    for (pitItr = pits.begin(); pitItr != pits.end(); ++pitItr){
        list<Location> neighbors = getNeighbors(*pitItr);

        list<Location>::iterator neighborItr;
        for (neighborItr = neighbors.begin(); neighborItr != neighbors.end(); ++neighborItr){
            lBreezes.remove(*neighborItr);
        }
    }

    return lBreezes.empty();
}
...
bool Agent::isInBounds(Location loc) {
    return loc.X > 0 && loc.X <= worldState.worldSize && loc.Y > 0 && loc.Y <= worldState.worldSize;
}
